...
from configs.default import dr_path
...
class DR_SingleDomain():
    def __init__(self, root_path=dr_path, domain_name='oia', split='total', train_transform=None):
        if domain_name in dr_name_dict.keys():
            ...
            raise ValueError('domain_name should be in oia, dohc, drtid')

        self.root_path = os.path.join(os.sep, root_path)
        self.split = split
        self.split_file = os.path.join(os.sep, root_path, f'{self.domain_name}_{split_dict[self.split]}' + '.txt')
        ...
        imgs, labels = DR_SingleDomain.read_txt(self.split_file, self.root_path)
...
class DR_FedDG():
    def __init__(self, test_domain='oia', batch_size=16):
        ...
        for domain_name in self.domain_list:
            self.site_dataloader_dict[domain_name], self.site_dataset_dict[domain_name] = DR_FedDG.SingleSite(
    ...
    def SingleSite(domain_name, batch_size=16):
        dataset_dict = {
            'train': DR_SingleDomain(domain_name=domain_name, split='train', train_transform=transform_train).dataset,
            'val': DR_SingleDomain(domain_name=domain_name, split='val').dataset,
            'test': DR_SingleDomain(domain_name=domain_name, split='total').dataset,
        }
