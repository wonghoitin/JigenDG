...
def get_argparse():
    ...
    parser.add_argument('--kd_epochs', help='epochs number', type=int, default=2)
    parser.add_argument('--alpha', help='alpha for KD', type=float, default=0.5)
    parser.add_argument('--temperature', help='temperature for KD', type=float, default=1.0)
    ...
    for i in range(args.comm + 1):
        ...
        for domain_name in dataobj.train_domain_list:
            for kd_epoch in range(args.kd_epochs):
                knowledge_distillation(model_dict[domain_name], global_model, dataloader_dict[domain_name]['train'],
                                       optimizer_dict[domain_name], metric, args)
